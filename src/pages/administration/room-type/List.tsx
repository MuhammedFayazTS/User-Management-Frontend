...
import { useDeleteRoomType, useGetRoomTypes } from "@/store/server/room-type";
...
// Filter fields
const filterFields: DataTableFilterField<RoomType>[] = [
  { value: "name", placeholder: "Search by name", label: "Name" },
];

const RoomTypeList: FC<IListProps> = ({ togglePage }) => {
  const [confirmDialog, setConfirmDialog] = useState<IConfirmDialog>()
  const [searchParams] = useSearchParams();
  const { reset: resetDatabaseId, setDatabaseId, databaseId, toggleViewPage } = useRoomTypeStore((state) => state);
  ...
  const { data, isLoading } = useGetRoomTypes({ search, sort, page, limit });
  ...
  const onConfirmDelete = async () => {
    assertDefined(databaseId, "Room Type id is not defined", true)
