...
import { useDeleteCustomer, useGetCustomers } from "@/store/server/customer";
...
const CustomerList: FC<IListProps> = ({ togglePage }) => {
  const [confirmDialog, setConfirmDialog] = useState<IConfirmDialog>()
  const [searchParams] = useSearchParams();
  const { reset: resetDatabaseId, setDatabaseId, databaseId, toggleViewPage } = useCustomerStore((state) => state);
  ...
  const { data, isLoading } = useGetCustomers({ search, sort, page, limit });
  ...
  const onConfirmDelete = async () => {
    assertDefined(databaseId, "Customer id is not defined", true)
