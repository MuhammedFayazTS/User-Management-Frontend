...
import { useDeleteRoomStatus, useGetRoomStatuses } from "@/store/server/room-status";
...
// Filter fields
const filterFields: DataTableFilterField<RoomStatus>[] = [
  { value: "name", placeholder: "Search by name", label: "Name" },
];

const RoomStatusList: FC<IListProps> = ({ togglePage }) => {
  const [confirmDialog, setConfirmDialog] = useState<IConfirmDialog>()
  const [searchParams] = useSearchParams();
  const { reset: resetDatabaseId, setDatabaseId, databaseId, toggleViewPage } = useRoomStatusStore((state) => state);
  ...
  const { data, isLoading } = useGetRoomStatuses({ search, sort, page, limit });
  const { mutate } = useDeleteRoomStatus();
  ...
  const onConfirmDelete = async () => {
    assertDefined(databaseId, "Room Status id is not defined", true)
  ...
  const columns: ColumnDef<RoomStatus>[] = [
    {
      accessorKey: "Sl No",
      cell: ({ row }) => {
