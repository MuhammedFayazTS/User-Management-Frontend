import { useEffect } from 'react';
import DefaultTextInput from '@/components/core/DefaultTextInput'
import { branchSchema } from './schema';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { Form } from '@/components/ui/form';
import { useAddBranch, useGetBranch, useUpdateBranch } from '@/store/server/branch';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import Layout from '@/components/core/Layout';
import DefaultTextArea from '@/components/core/DefaultTextArea';
import FormFooter from '@/components/core/FormFooter';
import { useHeaderContext } from '@/context/header-provider';
import { useBranchStore } from '@/store/client';
import SkeletonForm from '@/components/loaders/SkeletonForm';
import { handleMutationError, handleSuccessResponse } from '@/utils/handleMutationResponse';
import EditButton from '@/components/buttons/EditButton';
import { BaseApiResponse } from '@/types/common';
import { useGetCountriesForSelect } from '@/store/server/country';
import { DefaultSelect } from '@/components/core/DefaultSelect';

const defaultValues = {
    name: "",
    countryId: undefined,
    email: "",
    phone: "",
    address: "",
    city: "",
    postalCode: "",
    state: ""
}

const BranchForm = () => {
    const { reset: resetDatabaseId, toggleViewPage, databaseId, isViewPage } = useBranchStore((state) => state);

    const { mutate: addBranchMutation, isPending: isAddBranchPending } = useAddBranch();
    const { mutate: updateBranchMutation, isPending: isUpdateBranchPending } = useUpdateBranch();

    const { data: countriesData, isLoading: isCountriesLoading } = useGetCountriesForSelect();
    const { data: branchData, isLoading } = useGetBranch(databaseId ?? undefined);

    const { setIsLoading } = useHeaderContext()
    const formSchema = branchSchema()

    const form = useForm<z.infer<typeof formSchema>>({
        resolver: zodResolver(formSchema),
        defaultValues,
    });

    const { reset } = form;

    useEffect(() => {
        if (!isLoading && branchData?.branch) {
            reset(branchData.branch);
        }
        if (!databaseId) {
            reset(defaultValues)
        }
    }, [isLoading, branchData, reset,databaseId]);

    const successCallback = () => {
        reset();
        setIsLoading(false)
    }

    const errorCallback = () => {
        setIsLoading(false)
    }

    const mutationConfig = {
        onSuccess: (response: BaseApiResponse) => handleSuccessResponse(true, response?.message, successCallback),
        onError: (error: unknown) => handleMutationError(error, errorCallback),
    };

    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        setIsLoading(true)
        const updatedValues = { ...values }
        if (!databaseId) {
            addBranchMutation(updatedValues, mutationConfig);
        } else {
            updateBranchMutation({ id: databaseId, data: updatedValues }, mutationConfig);
        }
        await resetDatabaseId()
    };

    const onClickEditButton = () => toggleViewPage(false)

    return (
        <Card className={isLoading ? 'w-[650px]' : 'w-full'}>
            <CardHeader>
                <CardTitle>
                    {databaseId ? 'Update' : 'Create'} Branch
                    {isViewPage && <EditButton customClass='ml-3' onClick={onClickEditButton} />}
                </CardTitle>
            </CardHeader>
            <CardContent>
                {
                    isLoading ?
                        <SkeletonForm rows={3} fieldsPerRow={2} fieldWidth={450} fieldHeight={35} /> :
                        (<Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} onReset={() => reset()} className="space-y-8">
                                <Layout gap={5}>
                                    <Layout stack width={450}>
                                        <DefaultTextInput
                                            name='name'
                                            label='Name'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                        <DefaultTextInput
                                            name='state'
                                            label='State'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                        <DefaultTextInput
                                            name='postalCode'
                                            label='Postal Code'
                                            type='number'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                        <DefaultTextInput
                                            name='email'
                                            label='Email'
                                            type='email'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                    </Layout>
                                    <Layout stack width={450}>
                                        <DefaultSelect
                                            name='countryId'
                                            label='Country'
                                            isLoading={isCountriesLoading}
                                            options={countriesData?.countries}
                                            control={form.control}
                                            disabled={isViewPage}
                                        />
                                        <DefaultTextInput
                                            name='city'
                                            label='City'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                        <DefaultTextInput
                                            name='phone'
                                            label='Phone'
                                            type='number'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                        <DefaultTextArea
                                            name='address'
                                            label='Address'
                                            control={form.control}
                                            readOnly={isViewPage}
                                        />
                                    </Layout>
                                </Layout>
                                {!isViewPage && <FormFooter isSubmitting={isAddBranchPending || isUpdateBranchPending} />}
                            </form>
                        </Form>)}
            </CardContent>
        </Card>
    )
}

export default BranchForm