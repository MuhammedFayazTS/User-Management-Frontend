...
import { useDeleteBranch, useGetBranches } from "@/store/server/branch";
...
const BranchList: FC<IListProps> = ({ togglePage }) => {
  const [confirmDialog, setConfirmDialog] = useState<IConfirmDialog>()
  const [searchParams] = useSearchParams();
  const { reset: resetDatabaseId, setDatabaseId, databaseId, toggleViewPage } = useBranchStore((state) => state);
  ...
  const { data, isLoading } = useGetBranches({ search, sort, page, limit });
  ...
  const onConfirmDelete = async () => {
    assertDefined(databaseId, "Branch id is not defined", true)
